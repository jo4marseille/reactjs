...
export default function Account() {
    ...
    const updateUser = async (id) => {
        const res = await axios.put(URL_UPDATE + id, input);
        if (res) {
            alert("User updated");
        } else {
            alert("Error");
        ...
        <main style={{display: 'flex', flexDirection: 'column', textAlign: 'center', gap: '5vh'}}>
            ...
            <h3>Information</h3>
                {/* <form style={{display: 'flex', flexDirection: 'column', margin: 'auto', marginTop: '-3vh', gap: '1vh', backgroundColor: '#8080801f', borderRadius: '2vh', padding: '5vh'}}> */}
                    ...
                    <input style={{width: '100%', height: '3vh', borderRadius: '1vh', border: 'none', padding: '1vh'}} type="text" id="password" name="password" placeholder="password" onChange={(e) => setInput( ((prevInput)=>({ ...prevInput, password: e.target.value })))}/>
                    <button style={{padding: '0.5vh', backgroundColor: '#ffe37a'}} onClick={() => console.log(input)}>Update</button>
                {/* </form> */}
            <h3>My votes</h3>
            <ul style={{display: 'grid', gridTemplateColumns: 'repeat(4, 200px)', margin: 'auto', listStyle: 'none', marginTop: '-2vh'}}>
                {votes.map((vote) => (
                    <li key={vote.id} style={{border: 'solid', borderRadius: '1vh 1vh 3vh 3vh', padding: '3vh'}}>
                        <img src={vote.attributes.athlete.data.attributes.image} alt={vote.attributes.athlete.data.attributes.name} />
                        <p style={{margin: '2vh'}}>{vote.attributes.athlete.data.attributes.name}</p>
